...
| `lifecycle_mgr.py` | one-shot: configure + activate the smoother |
| `patrolbot-sh.p` | ARIA hardware profile (carried here too) |

Editor temp files and superseded experiments (`*~`, `#joy.xml#`, `rosaria2*.xml`, `teleop-key*.xml`,
`readlidar.py`) are [legacy/dead](../internals/legacy-components.md).
